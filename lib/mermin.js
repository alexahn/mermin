...
var mermin = {
    ...
    // taking out less type for now, test results were unstable
    types : {
        'js' : jsProcessor,
        'css' : cssProcessor,
        //'less' : lessProcessor
            ...
            depth_two_length = depth_two_keys.length; depth_two_index < depth_two_length; depth_two_index += 1) {
                ...
                file_length = file_array.length; file_index < file_length; file_index += 1) {
                    ...
                    if (!self.tree[type][category].output) {
                        ...
                        self.tree[type][category].output = basepath + path_arr.join('/');
                    }
                    if (fs.existsSync(basepath + file_path)) {
                        self.tree[type][category].paths.push(basepath + file_path);
                    }
                }
            }
        }
...
// taking out less processor for now, test results are unstable
/*
