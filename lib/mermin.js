...
var Mermin = function (opt) {
    ...
    self.middleware = function (req, res, next) {
        if (typeof res.local === 'function') {
            res.local(self.name, (self.merge) ? self.output : self.tree);
        }
        else if (typeof res.locals === 'function') {
            var inject = {};
            inject[self.name] = (self.merge) ? self.template : self.tree;
            res.locals(inject);
        }
        else {
            res.locals[self.name] = (self.merge) ? self.template : self.tree;
        }
        next();
    };
